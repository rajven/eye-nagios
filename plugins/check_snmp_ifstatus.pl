...
if ( $use_64bit ) {
  push( @snmpoids, $snmpIfHCInOctets . "." . $if_number );
  push( @snmpoids, $snmpIfHCOutOctets . "." . $if_number );
} else {
...
if ( $use_64bit ) {
  $in_bytes     = $response->{ $snmpIfHCInOctets    . "." . $if_number };
  $out_bytes    = $response->{ $snmpIfHCOutOctets   . "." . $if_number };
} else {
...
if ( $use_64bit ) {
  $in_bytes  = adjust_overflow( $in_bytes,  $old_in_bytes,  $max_counter64 );
  $out_bytes = adjust_overflow( $out_bytes, $old_out_bytes, $max_counter64 );
} else {
...
if ( $use_64bit ) {
  $warn_overflow = int($max_counter64 / ( $if_speed / 8 ));
}
